...
IFNULL(`object_from_name`.`name`, IFNULL(CONCAT((SELECT alias FROM object_alias WHERE id_object = object_from.id),' ',object_from.id),object_from.id)) AS `object_from`,
IFNULL(`object_to_name`.`name`, IFNULL(CONCAT((SELECT alias FROM object_alias WHERE id_object = object_to.id),' ',object_to.id),object_to.id)) AS `object_to`
FROM `object_correlation`
